...
def cycle_stats_from_signal(acc):
    acc = np.asarray(acc)
    if acc.size == 0: 
        return {'mean': np.nan, 'num_cycles':0, 'avg_amplitude':np.nan}
    mean_acc = np.mean(acc)
    peak_trough_idx, max_idx, min_idx = find_extrema(acc)
    ...
    if peak_trough_idx.size < 2:
        return {'mean': mean_acc, 'num_cycles':0, 'avg_amplitude':0.0}
    ...
    return {'mean': mean_acc, 'num_cycles': num_cycles, 'avg_amplitude': avg_amp}

# calculate per-axis stats 
results = []
for axis in ['InlineAcc','LateralAcc','VerticalAcc']:
    stats = cycle_stats_from_signal(df[axis].dropna().values)
    results.append({'Signal': axis, 'Mean_g': stats['mean'], 
                    'Num_cycles': stats['num_cycles'], 'Avg_amp_g': stats['avg_amplitude']})
...
results.append({'Signal':'Magnitude', 'Mean_g': stats['mean'],
                'Num_cycles': stats['num_cycles'], 'Avg_amp_g': stats['avg_amplitude']})
